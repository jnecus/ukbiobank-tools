...
def getFieldsInstancesArrays(ukb_csv=None, data_dict=None):
    ...
    for f in fieldnames:
        ...
        if f == "eid":
            ...
            field_instance, array_n = re.split(
                "\.", f
            )  # array_n is the number after the '.' (some fields have multiple measurements, e.g. multiple reported illnesses)
            field, instance = re.split("-", field_instance)
            temp_df = pd.DataFrame(
                {"field": field, "instance": instance, "array": array_n}, index=[0]
            )

            field_instance_array_df = field_instance_array_df.append(temp_df)

    field_instance_array_df[["field", "instance", "array"]] = field_instance_array_df[
        ["field", "instance", "array"]
    ].apply(pd.to_numeric)

    result = pd.merge(
        field_instance_array_df,
        data_dict[["FieldID", "Field"]],
        left_on="field",
        right_on="FieldID",
    )
    ...
    result["id_instance_array"] = (
        result["field_id"].astype(str)
        + "-"
        + result["instance"].astype(str)
        + "."
    ...
    result_duplicate["new_field_name"] = (
        result_duplicate["field_name"].astype(str)
        + " (Field ID: "
        + result_duplicate["field_id"].astype(str)
        + ")"
    )
    ...
    result2["field_name_instance_array"] = (
        result2["field_name"].astype(str)
        + "-"
        + result2["instance"].astype(str)
        + "."
        + result2["array"].astype(str)
    )
...
def getFieldIdsInstancesFromCategoryId(ukbio, field_ids=None):
    ...
    field_ids = field_instance_array_df_temp[
        field_instance_array_df_temp["field_id"].isin(field_ids)
    ]["id_instance_array"].tolist()
...
def getFieldIdsInstancesFromNamesInstances(ukbio, field_names=None):
    ...
    field_instance_array_df_temp = ukbio.field_instance_array_df.copy()
    field_ids = field_instance_array_df_temp[
        field_instance_array_df_temp["field_name_instance_array"].isin(field_names)
    ]["id_instance_array"].tolist()
...
def getFieldIdsFromNames(ukbio, field_names=None):
    ...
    field_instance_array_df_temp = ukbio.field_instance_array_df.copy()
    field_ids = field_instance_array_df_temp[
        field_instance_array_df_temp["field_name"].isin(field_names)
    ]["id_instance_array"].tolist()
...
def loadCsv(ukbio=None, fields=None, n_rows=None, instance=None):
    ...
    not_found = []
    all_field_names_and_ids = (
        ukbio.field_instance_array_df["field_name"].unique().tolist()
    )
    all_field_names_and_ids.extend(
        ukbio.field_instance_array_df["id_instance_array"].tolist()
    )
    ...
    all_field_names_and_ids.extend(
        ukbio.field_instance_array_df["field_name_instance_array"].tolist()
    )
    ...
    if len(field_id_to_convert) > 0:
        new_fields.extend(
            getFieldIdsInstancesFromCategoryId(ukbio, field_ids=field_id_to_convert)
        )
    ...
    if len(fieldname_to_convert) > 0:
        new_fields.extend(
            getFieldIdsInstancesFromNamesInstances(
                ukbio, field_names=fieldname_to_convert
            )
        )
    ...
    if instance is not None:
        df = ukbiobank.filtering.filterInstancesArrays(
            ukbio=ukbio, df=df, instances=instance
        )
    ...
    if ids is not None:
        ...
        for i in ids:
            ...
            # if string without instance-array appendige, then add appendages
                ...
                field_names = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_name"] == i
                ]["field_name"]
                field_instances = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_name"] == i
                ]["instance"]
                field_arrays = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_name"] == i
                ]["array"]
                fieldnames_instance_array = (
                    field_names
                    + "-"
                    + field_instances.astype(str)
                    + "."
                    + field_arrays.astype(str)
                )
            ...
            # else if field ID (int) and Field ID exists, then convert to name and add appendage
                ...
                field_names = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_id"] == int(i)
                ]["field_name"]
                field_instances = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_id"] == int(i)
                ]["instance"]
        ...
        for c in cols:
            if re.match("^[0-9]*-", c) is not None and c != "eid":
                ...
                fieldName = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_id"] == int(fieldID)
                ]["field_name"].iloc[0]
    ...
    for c in cols:
        if re.match("[0-9]*-[0-9]*\.[0-9]*$", c) is None and c != "eid":
            ...
            if c not in all_field_names:
                ...
                fieldNameParts = c.split("-")[
                    :-1
                ]  # get all parts of string up until the final '-' (i.e. the name)
                fieldName = "-".join(fieldNameParts)  #
                instanceArray = c.split("-")[-1]

                fieldID = fieldID_fieldName_df[
                    fieldID_fieldName_df["field_name"] == fieldName
                ]["field_id"].iloc[0]

                converted = re.sub(
                    "(.*)-[0-9]*\.[0-9]*$", str(fieldID) + "-" + instanceArray, c
                )
...
def illnessCodesToText(ukbio=None, df=None):
    ...
    nonCancerIllness.replace(
        to_replace=ukbio.nonCancerIllnessCoding.coding.tolist(),
        value=ukbio.nonCancerIllnessCoding.meaning.tolist(),
        inplace=True,
    )
    ...
    icd9.replace(
        to_replace=ukbio.icd9Coding.coding.tolist(),
        value=ukbio.icd9Coding.meaning.tolist(),
        inplace=True,
    )

    # Decode ICD10 illness
    icd10 = df.filter(
        regex="^(40002)|^(41270)|^(41202)|^(41204)|^(41201)|^(40006)|^(40001)"
    )
    icd10.replace(
        to_replace=ukbio.icd10Coding.coding.tolist(),
        value=ukbio.icd10Coding.meaning.tolist(),
        inplace=True,
    )
...
def addFields(ukbio=None, df=None, fields=None, instances=None):
    """
    ...
    instances: integer or list of integers, Optional
        If present, fields will be filtered for chosens instance(s) before being added.
    ...
    if instances is not None:
        new_df = ukbiobank.filtering.filterInstancesArrays(
            ukbio, df=new_df, instances=instances
        )
...
def calculateHealthySleepScore(ukbio=None, df=None, instances=[2, 3]):
    ...
    for instance in instances:
        ...
        hss = "healthy_sleep_score-{0}.0".format(
            instance
        )  # (hss : healthy sleep score)
...
def removeOutliers(df=None, std=3, cols=None):
    ...
    for c in cols:
        upper_limit, lower_limit = (df[c].mean() + 3 * df[c].std()), (
...
def calculateChangeInCognitiveScores(ukbio=None, df=None):
    ...
    cog_vars = [
        (
            "Mean time to correctly identify matches-3.0",
            "Mean time to correctly identify matches-2.0",
        ),
        (
            "Maximum digits remembered correctly (Field ID: 4282)-3.0",
            "Maximum digits remembered correctly (Field ID: 4282)-2.0",
        ),
        (
            "Fluid intelligence score (Field ID: 20016)-3.0",
            "Fluid intelligence score (Field ID: 20016)-2.0",
        ),
        (
            "Number of incorrect matches in round (Field ID: 399)-3.2",
            "Number of incorrect matches in round (Field ID: 399)-2.2",
        ),
        (
            "Number of puzzles correctly solved-3.0",
            "Number of puzzles correctly solved-2.0",
        ),
        ("Number of puzzles correct-3.0", "Number of puzzles correct-2.0"),
        (
            "Number of word pairs correctly associated-3.0",
            "Number of word pairs correctly associated-2.0",
        ),
        (
            "Duration to complete alphanumeric path (trail #2) (Field ID: 6350)-3.0",
            "Duration to complete alphanumeric path (trail #2) (Field ID: 6350)-2.0",
        ),
        (
            "Number of symbol digit matches made correctly (Field ID: 23324)-3.0",
            "Number of symbol digit matches made correctly (Field ID: 23324)-2.0",
        ),
    ]
...
def calculateCognitiveDeclineScore(
    ukbio=None, df=None, percentage_thresh=0, missing_tolerance=0
):
    """
    ...
    cog_vars = [
        "Percentage Change in Maximum digits remembered correctly (Field ID: 4282)",
    ...
    cog_vars_inverted = [
        "Percentage Change in Mean time to correctly identify matches",
    ...
    if not pd.Series(cog_vars + cog_vars_inverted).isin(df.columns).all():
        raise ValueError(
            "Change in cognitive scores not found in dataframe, try running calculateChangeInCognitiveScores() first"
        )
...
def meltByInstance(ukbio=None, df=None):
    ...
    df[["variable", "instance", "array"]] = df.variable.str.extract(
        pat="^(.*)-(.*)\.(.*)$"
    )
